...
contract Redemption is IRedemption, Ownable, ReentrancyGuard {
    ...
    /// @inheritdoc IRedemption
    uint256 public constant override INITIAL_ELIGIBLE_HASHES_TOTAL = 579;

    /// @inheritdoc IRedemption
    Hashes public immutable override HASHES;
    ...
    /// @notice Bitmap for excluded hash token IDs
    BitMaps.BitMap internal _excludedHashIDs;

    /// @notice Initializes the redemption contract with excluded token IDs and sets the owner
    /// @param _hashes Address of the hashes NFT contract
    ...
    constructor(Hashes _hashes, address _owner, uint256[] memory _excludedIds) {
        HASHES = _hashes;
        _transferOwnership(_owner);
        stage = Stages.PreRedemption;
        uint256 length = _excludedIds.length;
        for (uint256 i; i < length; i++) {
            _excludedHashIDs.set(_excludedIds[i]);
        }
        emit StageSet(stage);
    }
    ...
    function isHashEligibleForRedemption(uint256 _tokenId) public view override returns (bool) {
        ...
        if (_excludedHashIDs.get(_tokenId)) return false;
        return true;
    }
}
